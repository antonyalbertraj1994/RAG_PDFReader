...
}
{
  ...
  var onFileInputChange = async function (evt) {
    ...
    try {
      ...
      console.log("Saved Path:", result.path);
    } catch (error) {
      console.error("Error uploading PDF:", error);
      alert("Failed to upload PDF. Check console for details.");
    }

    // const text = "Antony ALbert Raj";
    ...
    console.log(`ReceivedServer:${data.summary}`)


  };
  var onOpenFile = function (evt) {
    console.log("File pressed12")
...
async function filepicker(file){
    console.log(`FileURL1:${file}`)
}
